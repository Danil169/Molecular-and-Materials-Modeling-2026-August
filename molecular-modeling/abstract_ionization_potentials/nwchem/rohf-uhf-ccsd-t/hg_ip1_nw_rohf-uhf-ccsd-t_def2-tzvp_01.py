...
def setup_directories():
    ...
    base_dir = "nwchem_tce_ccsdt_tzvp"
    subdirs = [
        "neutral",
        "cation_rohf",
        "cation_uhf"
...
def run_calculation_with_timing(atoms, calc_params, timer, section_name):
    """
    Run a calculation with timing tracking
    """
    # Start timing for SCF
    timer.start(f'{section_name}_scf')
    timer.start(f'{section_name}_total')
    
    # Run the calculation
    atoms.calc = NWChem(**calc_params)
    energy = atoms.get_potential_energy()
    
    # Stop timing for SCF (first part of calculation)
    timer.stop(f'{section_name}_scf')
    # The CCSD time will be counted separately via the timer
    timer.stop(f'{section_name}_total')
...
def run_tce_ccsdt_calculation(nproc, timer):
    ...
    hg_neutral.calc = NWChem(**neutral_calc_params)
    ...
    hg_cation_rohf.calc = NWChem(**cation_rohf_calc_params)
    ...
    hg_cation_uhf.calc = NWChem(**cation_uhf_calc_params)
