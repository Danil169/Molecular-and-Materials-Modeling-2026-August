...
- Cation Hg+: ROHF (restricted open-shell) and UHF (unrestricted open-shell)
...
def run_scf_calculation(nproc):
    ...
    # === Cation Hg+ (ROHF, open-shell doublet) ===
    print("\n--- Cation Hg+ (ROHF, Open-Shell Doublet) ---")
    cation_rohf_dir = os.path.join(base_dir, 'nwchem_cation_rohf')
    ...
    hg_cation_rohf.calc = NWChem(
        ...
        scf={
            'thresh': SCF_THRESH, 
            'maxiter': SCF_MAXITER,
            'rohf': True,       # Restricted Open-Shell Hartree-Fock
            'doublet': True     # Doublet spin state
        },
        memory=MEMORY,
        command=f"mpirun -np {nproc} nwchem PREFIX.nwi > PREFIX.nwo",
    )
    ...
    print("\n--- Cation Hg+ (UHF, Open-Shell Doublet) ---")
    cation_uhf_dir = os.path.join(base_dir, 'nwchem_cation_uhf')
    ...
    hg_cation_uhf.calc = NWChem(
        ...
        scf={
            'thresh': SCF_THRESH, 
            'maxiter': SCF_MAXITER,
            'uhf': True,        # Unrestricted Hartree-Fock
            'doublet': True     # Doublet spin state
        },
        memory=MEMORY,
        command=f"mpirun -np {nproc} nwchem PREFIX.nwi > PREFIX.nwo",
    ...
    print(f"Cation Energy (ROHF):           {e_cation_rohf:>12.6f} eV")
    print(f"Cation Energy (UHF):            {e_cation_uhf:>12.6f} eV")
    print("-"*70)
    print(f"Vertical IE (ROHF cation):      {ie_rohf:>12.6f} eV")
    print(f"Vertical IE (UHF cation):       {ie_uhf:>12.6f} eV")
    print(f"Experimental IE:                {EXP_IE:>12.4f} eV")
    print("-"*70)
    print(f"Error (ROHF cation):            {ie_rohf-EXP_IE:>+12.4f} eV")
    print(f"Error (UHF cation):             {ie_uhf-EXP_IE:>+12.4f} eV")
    print("="*70)
    ...
    print(f"   ├── {os.path.basename(cation_rohf_dir)}/  (ROHF, open-shell)")
    print(f"   └── {os.path.basename(cation_uhf_dir)}/   (UHF, open-shell)")
...
def print_file_summary(results):
    ...
    if results:
        ...
        if os.path.exists(cation_rohf_dir):
            ...
            print(f"\n   {os.path.basename(cation_rohf_dir)}/ (ROHF):")
            for f in sorted(files):
        ...
        if os.path.exists(cation_uhf_dir):
            ...
            print(f"\n   {os.path.basename(cation_uhf_dir)}/ (UHF):")
            for f in sorted(files):
...
def main():
    ...
    print("With ROHF and UHF for cation (RHF is only for closed-shell)")
    print("="*70)
    ...
    print("="*70)
    print(f"✅ NWChem SCF-ECP calculations completed")
    print(f"\n   ROHF cation IE = {results['ie_rohf']:.6f} eV (error: {results['ie_rohf']-EXP_IE:+.6f} eV)")
    print(f"   UHF cation IE  = {results['ie_uhf']:.6f} eV (error: {results['ie_uhf']-EXP_IE:+.6f} eV)")
    print(f"\n   • Method: SCF (Hartree-Fock)")
    print(f"   • Neutral: RHF (closed-shell, singlet)")
    print(f"   • Cation ROHF: restricted open-shell (doublet)")
    print(f"   • Cation UHF:  unrestricted open-shell (doublet)")
