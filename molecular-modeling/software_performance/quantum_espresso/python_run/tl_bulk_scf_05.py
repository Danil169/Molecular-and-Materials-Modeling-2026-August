...
def find_mpirun_path():
    """Find the full path to mpirun."""
    mpirun = shutil.which('mpirun')
    if mpirun:
        return mpirun
    # Try mpiexec as fallback
    mpiexec = shutil.which('mpiexec')
    if mpiexec:
        return mpiexec
    return None

def get_ldd_info(executable):
    """Get ldd (shared library dependencies) information for an executable."""
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if ldd_path:
                result = subprocess.run([ldd_path, executable], capture_output=True, text=True)
                if result.returncode == 0:
                    return result.stdout
            return None
    except Exception as e:
        return f"Error running ldd: {e}"
...
def print_executable_info(pw_path, mpirun_path=None):
    """
    Print detailed information about executables including full paths and ldd.
    """
    print("=" * 80)
    ...
    if os.path.exists(pw_path):
        ...
        if ldd_output:
            # Indent the ldd output for better readability
            for line in ldd_output.split('\n'):
                if line.strip():
                    print(f"     {line}")
        else:
            print("     (ldd information not available)")
    else:
        print(f"   Exists: No")
...
def main():
    ...
    # Find mpirun path
    mpirun_path = find_mpirun_path()
    
    # Print executable information with full paths and ldd
    print_executable_info(pw_executable, mpirun_path)
    ...
    for idx, (nproc, omp_threads) in enumerate(combinations, 1):
        ...
        # Determine which mpirun command to use
        mpirun_cmd = 'mpirun'
        if mpirun_path:
            mpirun_cmd = mpirun_path
        
        result = run_benchmark(profile, input_data, pseudopotentials, atoms, nproc, omp_threads, mpirun_cmd=mpirun_cmd)
        if result:
